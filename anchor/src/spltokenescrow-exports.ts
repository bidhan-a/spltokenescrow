...
export const SPLTOKENESCROW_PROGRAM_ID = new PublicKey(
  SpltokenescrowIDL.address
);

// This is a helper function to get the Spltokenescrow Anchor program.
export function getSpltokenescrowProgram(
  ...
  return new Program(
    {
      ...SpltokenescrowIDL,
      address: address ? address.toBase58() : SpltokenescrowIDL.address,
    } as Spltokenescrow,
    provider
  );
}
...
export function getSpltokenescrowProgramId(cluster: Cluster) {
  switch (cluster) {
    ...
    case "testnet":
      ...
      return new PublicKey("u4AZsQHiYE5wZcHkaoW6WKkypg7uTXVQuok3Ka9ghci");
    case "mainnet-beta":
    default:
